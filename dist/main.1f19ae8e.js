...
var div = dom.create("<div>newDiv</div>"); //但是有bug，将内容改成<td>hi</td>,就出现undefine

console.log(div); //因为这里没有引入div，所以需要去html做把JS引入进来

dom.after(test, div);
var div3 = dom.create('<div id = "parent"> </div>'); //创建div3
...
var title = dom.attr(test, 'title'); //用attr这个属性获取这个title这个的属性，test,'title'属性放在const title变量里面

console.log("title:".concat(title)); //这里的``是tap上面的符号
...
var testDiv = dom.find('#test')[0]; //不获取第0个，会是个数组，而不是div;这里是引号''
...
var t = dom.find('#travel')[0]; //遍历的元素是t,没加0，返回的就是一个数组

dom.each(dom.children(t), function (n) {
  return dom.style(n, 'color', 'red');
}); //遍历t的每一个子元素,对应每个元素进行站位n，每一个元素的颜色变成红色
